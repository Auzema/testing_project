...
try {
    ...
    registerUser(baseUrl, "empty_body_${runId}", "empty_body_${runId}@example.com", password, timeout, pause)
    ...
    WebUI.setText(xpath('Article title input', "//input[@placeholder='Article Title']"), "Empty Body Article ${runId}")
    pause()
    WebUI.setText(xpath('Article description input', "//input[@placeholder=\"What's this article about?\"]"), 'Description for empty body test')
    pause()
    WebUI.setText(xpath('Tag input', "//input[@placeholder='Enter tags']"), "emptybody${runId}")
    pause()
    ...
    WebUI.waitForElementVisible(
        xpath('Body blank error', "//*[contains(@class,'error-messages')]//li[contains(.,'body') and contains(.,'blank')]"),
        timeout
    )
} finally {
    WebUI.closeBrowser()
