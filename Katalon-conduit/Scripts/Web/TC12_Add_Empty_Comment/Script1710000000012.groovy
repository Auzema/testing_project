...
void createArticle(String title, String runId, int timeout, Closure pause) {
    ...
    pause()
    WebUI.setText(xpath('Article description input', "//input[@placeholder=\"What's this article about?\"]"), 'Article for empty comment test')
    pause()
    WebUI.setText(xpath('Article body textarea', "//textarea[@placeholder='Write your article (in markdown)']"), 'Body for empty comment test')
    pause()
    WebUI.setText(xpath('Tag input', "//input[@placeholder='Enter tags']"), "emptycomment${runId}")
    pause()
...
String title = "Empty Comment Article ${runId}"
...
try {
    ...
    registerUser(baseUrl, "empty_comment_${runId}", "empty_comment_${runId}@example.com", password, timeout, pause)
    ...
    WebUI.verifyElementNotPresent(xpath('Any created comment', "//*[contains(@class,'card-text')]"), 3)
    WebUI.verifyElementVisible(xpath('Comment textarea still visible', "//textarea[@placeholder='Write a comment...']"))
} finally {
    WebUI.closeBrowser()
}
