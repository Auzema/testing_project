...
String title = "Katalon Tagged Article ${runId}"
String description = "Article for tag filter automation"
String body = "This article is used for validating tag filtering."
String tag = "filtertag${runId}"
...
try {
    ...
    WebUI.navigateToUrl(baseUrl)
    pause()
    WebUI.waitForElementVisible(xpath('Popular Tags label', "//*[normalize-space()='Popular Tags']"), timeout)
    WebUI.waitForElementVisible(xpath('Generated tag link', "//div[contains(@class,'sidebar')]//a[normalize-space()='${tag}']"), timeout)
    WebUI.click(xpath('Generated tag link', "//div[contains(@class,'sidebar')]//a[normalize-space()='${tag}']"))
    pause()

    WebUI.waitForElementVisible(xpath('Active tag tab', "//a[contains(@class,'active') and contains(normalize-space(.),'${tag}')]"), timeout)
    WebUI.waitForElementVisible(xpath('Created article in filtered list', "//a[contains(@class,'preview-link')]//h1[normalize-space()='${title}']"), timeout)
} finally {
    WebUI.closeBrowser()
}
